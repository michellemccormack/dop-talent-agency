...
// Streams GPT tokens to the browser and persists turns to a "sessions" store.

const path = require("path");
const fs = require("fs/promises");
const { stream } = require("@netlify/functions");

// Prefer widely-available Blobs API (getStore). Fallback to in-memory.
let getStore;
try {
  ...
  return stream(event, (res) => {
    ...
    (async () => {
      try {
        ...
        // session load/init
        let session = (await sessions.get(sessionId).catch(() => null)) || {
          id: sessionId, messages: [], persona: null, createdAt: Date.now(), updatedAt: Date.now()
        };
        ...
        const body = { model: DEFAULT_MODEL, stream: true, messages, temperature: 0.7 };
